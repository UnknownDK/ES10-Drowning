...
WORD_ALIGNED_ATTR uint8_t recvbuf[2688];
uint8_t sendbuf[2688];
...
void app_main(void)
{
    ...
    dsps_fir_init_f32(&fir_ula1, FIR1WEIGHTS, delay_line_ula1, fir_len_ula); 
    float *fir_out_ula2 = (float *)malloc( N_buff * sizeof(float));
    dsps_fir_init_f32(&fir_ula2, FIR2WEIGHTS, delay_line_ula2, fir_len_ula); 
    float *fir_out_ula3 = (float *)malloc( N_buff * sizeof(float));
    dsps_fir_init_f32(&fir_ula3, FIR3WEIGHTS, delay_line_ula3, fir_len_ula); 
    ...
    while (1) {
        ...
        */
        ...
        for (int i = 0; i < SAMPLES_PER_MIC*NUMBER_OF_MICS*3 - 1; i +=3){ // 2688 - last int is number of bytes per sample (24 bit = 3)
            ...
            mic_inputs[i/3] = (float)concatenated_data / pow(2,31);
        }
        ...
        dsps_fir_f32(&fir_ula1, ula1_output_summed, fir_out_ula1, FIR1LENGTH + SAMPLES_PER_MIC);
        ...
        dsps_fir_f32(&fir_ula2, ula2_output_summed, fir_out_ula2, FIR2LENGTH + SAMPLES_PER_MIC);
        //printf("Fir2: %ld\n", dsp_get_cpu_cycle_count()-firstart);
        ...
        dsps_fir_f32(&fir_ula3, ula3_output_summed, fir_out_ula3, FIR3LENGTH + SAMPLES_PER_MIC);
        //printf("Fir3: %ld\n", dsp_get_cpu_cycle_count()-firstart);
